...
export class NGCF {
  ...
  constructor(numUsers: number, numItems: number, R: tf.Tensor2D, layers = NUM_LAYERS, embeddingDim = EMBEDDING_DIM) {
    ...
    for (let l = 0; l < layers; l++) {
      this.W1.push(tf.variable(tf.randomNormal([embeddingDim, embeddingDim], 0, 0.1)));
      this.W2.push(tf.variable(tf.randomNormal([embeddingDim, embeddingDim], 0, 0.1)));
    }
  ...
  buildAdjacencyMatrix(): tf.Tensor2D {
    ...
    return tf.concat([
      tf.concat([zeroUU, this.R], 1),
      tf.concat([RT, zeroII], 1)
    ], 0) as tf.Tensor2D;
  }
  ...
  applyNodeDropout(L: tf.Tensor2D, dropRate: number): tf.Tensor2D {
    ...
    const mask = tf.randomUniform([numNodes]).greaterEqual(dropRate) as tf.Tensor1D;
  ...
  propagate(): tf.Tensor2D {
    ...
    let Ldrop = NODE_DROPOUT_RATIO > 0 ? this.applyNodeDropout(this.L, NODE_DROPOUT_RATIO) : this.L;
    ...
    for (let l = 0; l < this.layers; l++) {
      ...
      if (MESSAGE_DROPOUT_RATIO > 0) E_new = tf.dropout(E_new, MESSAGE_DROPOUT_RATIO);
